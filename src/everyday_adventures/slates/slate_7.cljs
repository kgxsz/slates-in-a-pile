(ns everyday-adventures.slates.slate-7
  (:require [om.core :as om :include-macros true]
            ...
            [everyday-adventures.slates.slate-utils :refer [create-element-join
                                                            ...
                                                            create-text-join]]))
(defn select-canvas []
  (.select js/d3 "#slate-7 .canvas"))

(defn initialize-canvas []
  (-> (.select js/d3 "#slate-7 .slate-content")
      (.append "svg")
      (.attr "class" "canvas")
      (.attr "width" 1088)
      (.attr "height" 450)))
...
(defn vectorize-constructs [constructs-data]
  (let [select-by-id (fn [id] (.select js/d3 (str "#slate-7 #" id)))]
...
(defcomponent slate-7 [{:keys [step] :as cursor} owner]
  (did-mount [_]
    (.log js/console "slate 7 mounted with step: " step)
    (initialize-canvas)
    (initialize-statics)
    (initialize-dynamics step))

  (did-update [_ _ _]
    (.log js/console "slate 7 updated with step: " step)
